...
// Media preview component with fallback for images and video thumbnails
function MediaPreview({ 
  ...
  // For video files, try to show thumbnail first
  if (isVideo) {
    return (
      <img
        src={`/api/files/thumbnail/${fileId}`}
...
export default function FileGrid({ 
  ...
  onShare, 
  onRefresh, 
  formatFileSize: formatFileSizeFunc = formatFileSize,
  viewMode = 'grid',
  onViewModeChange,
  ...
  const getFileIconColor = (mimeType: string) => {
    ...
    if (mimeType.includes('pdf') || mimeType.includes('document')) return 'text-red-600';
      ...
      {files.map((file) => {
        ...
        return (
          <Link key={file.id} href={`/file/${file.id}`}>
            <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-md hover:shadow-lg rounded-2xl transition-all duration-300 cursor-pointer overflow-hidden mb-4">
              <CardContent className="p-0">
                <div className="flex items-center p-4">
                  {/* Thumbnail */}
                  ...
                  <div className="flex items-center space-x-3 sm:space-x-2 ml-4">
                    <Button
                      variant="ghost"
                      size="sm"
                      className="h-10 w-10 sm:h-8 sm:w-8 p-0 text-gray-600 hover:text-blue-600 hover:bg-blue-50 rounded-full"
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        onShare && onShare(file);
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      className="h-10 w-10 sm:h-8 sm:w-8 p-0 text-gray-600 hover:text-green-600 hover:bg-green-50 rounded-full"
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        handleDownload(file);
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      className="h-10 w-10 sm:h-8 sm:w-8 p-0 text-gray-600 hover:text-red-600 hover:bg-red-50 rounded-full"
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        handleDelete(file);
